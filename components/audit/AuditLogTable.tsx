...
export function AuditLogTable() {
  ...
  const handleExportCSV = async () => {
    ...
    try {
      ...
      link.setAttribute(
        'download',
        `GPMS_AuditLogs_${new Date().toISOString().split('T')[0]}.csv`
      );
  ...
  const getModuleColor = (moduleName: string) => {
    switch (moduleName) {
      case 'Users':
        return 'bg-ink-glow/10 text-[#32234A] border-[#32234A]/20';
      case 'Auth':
        return 'bg-gold-soft/10 text-[#B4823A] border-[#B4823A]/20';
      case 'Donations':
      ...
      default:
        return 'bg-muted-ink/10 text-muted-ink border-muted-ink/20';
    }
  };
                ...
                return (
                  <React.Fragment key={log.logId}>
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`px-2 py-0.5 rounded-[6px] text-[12px] font-bold border ${getModuleColor(log.module)}`}
                        >
                          {log.module || 'System'}
                        </span>
                      </td>
                    ...
                    {isExpanded && hasDetails && (
                      <tr className="bg-cream-2 border-t-0 border-b border-hair">
                        <td
                          colSpan={6}
                          className="px-4 py-4 pl-12 border-b border-hair"
                        >
