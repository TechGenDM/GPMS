...
function runDonationTests() {
  ...
  var res1 = JSON.parse(DonationService.create(testAdminUser, donationPayload).getContent());
  ...
  var res2 = JSON.parse(DonationService.get(testAdminUser, { donationId: createdDonationId }).getContent());
  ...
  var res3 = JSON.parse(DonationService.search(testAdminUser, { category: "General" }).getContent());
  if (res3.success && res3.data.length > 0) {
    var found = res3.data.some(function(d) { return d.id === createdDonationId; });
    if (found) {
  ...
  var res4 = JSON.parse(DonationService.update(testAdminUser, updatePayload).getContent());
  if (res4.success) {
    var checkRes = JSON.parse(DonationService.get(testAdminUser, { donationId: createdDonationId }).getContent());
  ...
  var res5 = JSON.parse(DonationService.cancel(testAdminUser, { donationId: createdDonationId }).getContent());
  if (res5.success) {
    var checkRes2 = JSON.parse(DonationService.get(testAdminUser, { donationId: createdDonationId }).getContent());
  ...
  var res6 = JSON.parse(dispatch(simulatedPostData.action, simulatedPostData.payload).getContent());
  if (res6.success && res6.data.id === createdDonationId) {
    Logger.log("✅ Passed: Global dispatch correctly authenticated user and routed the request.");
  } else {
    Logger.log("❌ Failed: Global dispatch failed - " + JSON.stringify(res6));
  }

  Logger.log("=== Donation Tests Complete ===");
  Logger.log("Please check your Donations, Metadata, and AuditLogs sheets in Google Sheets.");
}
