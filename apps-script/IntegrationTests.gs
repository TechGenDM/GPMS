...
function runIntegrationTests() {
  ...
  var res1 = JSON.parse(
    UserService.authenticate({ email: 'unknown@example.com' }).getContent()
  );
  ...
  var res3 = JSON.parse(
    UserService.authenticate({ email: testEmail }).getContent()
  );
  ...
  if (res4.success === true) {
    var checkUser = JSON.parse(
      UserService.findById(createdUserId).getContent()
    );
  ...
  Logger.log(
    'Please check your Users, Metadata, and AuditLogs sheets in Google Sheets to confirm rows were inserted and logs were captured.'
  );
}
