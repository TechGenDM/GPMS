...
function runDashboardTests() {
  ...
  if (!authRes.success) {
    Logger.log(
      '❌ Failed to authenticate test admin. Run runDonationTests first.'
    );
    return;
  }
  ...
  var summaryRes = JSON.parse(
    DashboardService.getFinancialSummary(testAdminUser).getContent()
  );
  if (
    summaryRes.success &&
    summaryRes.data.donations !== undefined &&
    summaryRes.data.balance !== undefined
  ) {
  ...
  var activityRes = JSON.parse(
    DashboardService.getRecentActivity(testAdminUser, { limit: 5 }).getContent()
  );
  if (activityRes.success && Array.isArray(activityRes.data)) {
    Logger.log(
      '✅ Passed: Recent Activity retrieved ' +
    ...
    if (activityRes.data.length > 0) {
      Logger.log(
        '   Latest activity type: ' +
          activityRes.data[0].type +
          ' - ' +
          activityRes.data[0].title
      );
    }
  } else {
  ...
  var donStatsRes = JSON.parse(
    DashboardService.getDonationStats(testAdminUser).getContent()
  );
  ...
  var expStatsRes = JSON.parse(
    DashboardService.getExpenseStats(testAdminUser).getContent()
  );
  ...
  var pubRes = JSON.parse(
    dispatch(simulatedPostData.action, simulatedPostData.payload).getContent()
  );
  if (
    pubRes.success &&
    pubRes.data.committeeName !== undefined &&
    pubRes.data.totalCollection !== undefined
  ) {
    Logger.log(
      '✅ Passed: Public Dashboard successfully bypassed auth and retrieved stripped data.'
    );
    Logger.log(
      '   Committee: ' +
        pubRes.data.committeeName +
        ' | Balance: ' +
        pubRes.data.balance
    );
  } else {
    Logger.log(
      '❌ Failed: Public Dashboard blocked or failed. ' + JSON.stringify(pubRes)
    );
  }
