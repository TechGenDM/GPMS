...
function runExpenseTests() {
  ...
  var resInvalid = JSON.parse(
    ExpenseService.create(testAdminUser, invalidPayload).getContent()
  );
  if (!resInvalid.success && resInvalid.code === ERROR_CODES.INVALID_CATEGORY) {
    Logger.log('✅ Passed: Rejected invalid category correctly.');
  } else {
    Logger.log(
      '❌ Failed: Did not reject invalid category as expected. ' +
        JSON.stringify(resInvalid)
    );
  }
  ...
  var resAmt = JSON.parse(
    ExpenseService.create(testAdminUser, invalidAmtPayload).getContent()
  );
  ...
  var resCat = JSON.parse(
    ExpenseService.create(testAdminUser, {
      description: 'Taxi',
      amount: 500,
    }).getContent()
  );
  ...
  var res1 = JSON.parse(
    ExpenseService.create(testAdminUser, expensePayload).getContent()
  );
  ...
  var res2 = JSON.parse(
    ExpenseService.get(testAdminUser, {
  ...
  var res3 = JSON.parse(
    ExpenseService.search(testAdminUser, { vendor: 'Uber' }).getContent()
  );
  if (res3.success && res3.data.length > 0) {
    var found = res3.data.some(function (e) {
      return e.id === createdExpenseId;
    });
    if (found) {
  ...
  var res4 = JSON.parse(
    ExpenseService.update(testAdminUser, updatePayload).getContent()
  );
  if (res4.success) {
  ...
  if (res5.success) {
    ...
    if (
      checkRes2.data.status === CONFIG.status.cancelled &&
      checkRes2.data.remarks === 'Wrong amount entered'
    ) {
