...
function runFinancialConsistencyTests() {
  ...
  for (var i = 1; i < categoriesData.length; i++) {
    if (categoriesData[i][0] === "Transportation" && categoriesData[i][1] === "Expense") expenseCatExists = true;
    if (categoriesData[i][0] === "General" && categoriesData[i][1] === "Donation") donationCatExists = true;
  }
  ...
  var donRes = JSON.parse(DonationService.create(testAdminUser, donPayload).getContent());
  if (!donRes.success) {
  ...
  if (balanceAfterDonation === initialBalance + 500) {
    ...
    Logger.log("❌ Failed: Balance is " + balanceAfterDonation + ", expected " + (initialBalance + 500));
  }
  ...
  var expRes = JSON.parse(ExpenseService.create(testAdminUser, expPayload).getContent());
  if (!expRes.success) {
  ...
  if (balanceAfterExpense === initialBalance + 300) {
    ...
    Logger.log("❌ Failed: Balance is " + balanceAfterExpense + ", expected " + (initialBalance + 300));
  }

  // 3. Cancel the Expense
  Logger.log("3. Cancelling the Expense...");
  var cancelRes = JSON.parse(ExpenseService.cancel(testAdminUser, { expenseId: expenseId, reason: "Testing refund" }).getContent());
  ...
  if (balanceAfterCancel === initialBalance + 500) {
    Logger.log("✅ Passed: Balance restored to ₹500 diff after expense cancellation.");
  } else {
    Logger.log("❌ Failed: Balance is " + balanceAfterCancel + ", expected " + (initialBalance + 500));
  }
