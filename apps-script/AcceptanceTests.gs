...
function _assert(testName, condition, failureMessage) {
  ...
  if (condition) {
    ...
    _testResults.failed++;
    _testResults.failures.push(testName + ": " + (failureMessage || "Assertion failed"));
    Logger.log("  ❌ " + testName + " — " + (failureMessage || "Assertion failed"));
  }
}
...
function _setupTestEnvironment() {
  ...
  // 1. Create Admin
  var adminRes = _parseResponse(UserService.createUser({
    fullName: "Test Admin",
  ...
  if (adminRes.success) {
    var authAdmin = _parseResponse(UserService.authenticate({ email: "testadmin@gpms.org" }));
    ...
    var authAdmin2 = _parseResponse(UserService.authenticate({ email: "testadmin@gpms.org" }));
    _testUsers.admin = authAdmin2.data;
    Logger.log("  Admin reused: " + _testUsers.admin.id);
  }

  // 2. Create Volunteer
  var volRes = _parseResponse(UserService.createUser({
  ...
  if (volRes.success) {
    var authVol = _parseResponse(UserService.authenticate({ email: "testvolunteer@gpms.org" }));
    ...
    var authVol2 = _parseResponse(UserService.authenticate({ email: "testvolunteer@gpms.org" }));
    _testUsers.volunteer = authVol2.data;
    Logger.log("  Volunteer reused: " + _testUsers.volunteer.id);
  }

  // 3. Create Viewer (low-privilege user)
  var viewerRes = _parseResponse(UserService.createUser({
  ...
  if (viewerRes.success) {
    var authViewer = _parseResponse(UserService.authenticate({ email: "testviewer@gpms.org" }));
    ...
    var authViewer2 = _parseResponse(UserService.authenticate({ email: "testviewer@gpms.org" }));
  ...
  neededSettings.forEach(function(s) {
    ...
    for (var i = 1; i < setData.length; i++) {
      if (setData[i][0] === s[0]) { exists = true; break; }
    }
...
function _runGroupA() {
  ...
  // A1: Create Donation
  var donRes = _parseResponse(DonationService.create(admin, {
    donorName: "Func Test Donor",
    amount: 1000,
    category: "General",
    paymentMode: "Cash"
  }));
  _assert("A1: Create Donation", donRes.success && donRes.data.id, donRes.message);
  if (donRes.success) _testDonIds.push(donRes.data.id);

  // A2: Get Donation
  var getRes = _parseResponse(DonationService.get(admin, { donationId: _testDonIds[0] }));
  _assert("A2: Get Donation", getRes.success && getRes.data.donorName === "Func Test Donor", getRes.message);

  // A3: Update Donation (Admin)
  var updRes = _parseResponse(DonationService.update(admin, { donationId: _testDonIds[0], amount: 2000 }));
  _assert("A3: Update Donation", updRes.success, updRes.message);

  // A4: Search Donation
  var searchRes = _parseResponse(DonationService.search(admin, { donorName: "Func Test" }));
  _assert("A4: Search Donation", searchRes.success && searchRes.data.length > 0, searchRes.message);

  // A5: Create Expense
  var expRes = _parseResponse(ExpenseService.create(admin, {
    category: "Decoration",
    description: "Test flowers",
    amount: 300,
    vendor: "Local Shop"
  }));
  _assert("A5: Create Expense", expRes.success && expRes.data.id, expRes.message);
  if (expRes.success) _testExpIds.push(expRes.data.id);

  // A6: Get Expense
  var getExpRes = _parseResponse(ExpenseService.get(admin, { expenseId: _testExpIds[0] }));
  _assert("A6: Get Expense", getExpRes.success && getExpRes.data.description === "Test flowers", getExpRes.message);

  // A7: Update Expense (Admin)
  var updExpRes = _parseResponse(ExpenseService.update(admin, { expenseId: _testExpIds[0], amount: 500 }));
  _assert("A7: Update Expense", updExpRes.success, updExpRes.message);

  // A8: Search Expense
  var srchExpRes = _parseResponse(ExpenseService.search(admin, { vendor: "Local" }));
  _assert("A8: Search Expense", srchExpRes.success && srchExpRes.data.length > 0, srchExpRes.message);

  // A9: Dashboard Summary
  var dashRes = _parseResponse(DashboardService.getDashboardSummary(admin));
  _assert("A9: Dashboard Summary", dashRes.success && dashRes.data.summary.balance !== undefined, dashRes.message);

  // A10: Recent Activity
  var actRes = _parseResponse(DashboardService.getRecentActivity(admin, { limit: 5 }));
  _assert("A10: Recent Activity", actRes.success && Array.isArray(actRes.data), actRes.message);

  // A11: Cancel Donation
  var cancelDonRes = _parseResponse(DonationService.cancel(admin, { donationId: _testDonIds[0] }));
  _assert("A11: Cancel Donation", cancelDonRes.success, cancelDonRes.message);

  // A12: Cancel Expense
  var cancelExpRes = _parseResponse(ExpenseService.cancel(admin, { expenseId: _testExpIds[0], reason: "Test cancel" }));
  _assert("A12: Cancel Expense", cancelExpRes.success, cancelExpRes.message);

  // A13: Verify cancelled donation status
  var checkDon = _parseResponse(DonationService.get(admin, { donationId: _testDonIds[0] }));
  _assert("A13: Donation status is Cancelled", checkDon.data.status === CONFIG.status.cancelled,
    "Expected Cancelled, got " + checkDon.data.status);

  // A14: Verify cancelled expense status
  var checkExp = _parseResponse(ExpenseService.get(admin, { expenseId: _testExpIds[0] }));
  _assert("A14: Expense status is Cancelled", checkExp.data.status === CONFIG.status.cancelled,
...
function _runGroupB() {
  ...
  var volDon = _parseResponse(DonationService.create(volunteer, {
    donorName: "Vol Donor", amount: 100, category: "General", paymentMode: "UPI"
  }));
  ...
  if (volDonId) {
    var volCancel = _parseResponse(DonationService.cancel(volunteer, { donationId: volDonId }));
    _assert("B2: Volunteer CANNOT cancel donation", !volCancel.success && volCancel.code === ERROR_CODES.ROLE_NOT_ALLOWED,
      "Expected ROLE_NOT_ALLOWED, got " + volCancel.code);
  }

  // B3: Volunteer tries to update donation (SHOULD FAIL)
  if (volDonId) {
    var volUpdate = _parseResponse(DonationService.update(volunteer, { donationId: volDonId, amount: 999 }));
    _assert("B3: Volunteer CANNOT update donation", !volUpdate.success && volUpdate.code === ERROR_CODES.ROLE_NOT_ALLOWED,
      "Expected ROLE_NOT_ALLOWED, got " + volUpdate.code);
  }

  // B4: Volunteer tries to cancel expense (SHOULD FAIL)
  var volExp = _parseResponse(ExpenseService.create(volunteer, {
    category: "Decoration", description: "Vol test", amount: 50
  }));
  if (volExp.success) {
    var volExpCancel = _parseResponse(ExpenseService.cancel(volunteer, { expenseId: volExp.data.id, reason: "test" }));
    _assert("B4: Volunteer CANNOT cancel expense", !volExpCancel.success && volExpCancel.code === ERROR_CODES.ROLE_NOT_ALLOWED,
      "Expected ROLE_NOT_ALLOWED, got " + volExpCancel.code);
  }

  // B5: Unknown email authentication (SHOULD FAIL)
  var unknownAuth = _parseResponse(UserService.authenticate({ email: "nobody@fake.com" }));
  _assert("B5: Unknown email → USER_NOT_FOUND", !unknownAuth.success && unknownAuth.code === ERROR_CODES.USER_NOT_FOUND,
    "Expected USER_NOT_FOUND, got " + unknownAuth.code);

  // B6: Disabled user authentication (SHOULD FAIL)
  // Create a disabled user
  var disabledRes = _parseResponse(UserService.createUser({
    fullName: "Disabled User", email: "disabled@gpms.org", phone: "9000000099",
    role: CONFIG.roles.volunteer, status: CONFIG.status.active
  }));
  if (disabledRes.success || disabledRes.code === ERROR_CODES.EMAIL_ALREADY_EXISTS) {
    // Disable the user
    var disAuth = _parseResponse(UserService.authenticate({ email: "disabled@gpms.org" }));
    if (disAuth.success) {
      UserService.disableUser({ userId: disAuth.data.id, adminUserId: admin.id });
      // Now try to authenticate
      var disabledAuth = _parseResponse(UserService.authenticate({ email: "disabled@gpms.org" }));
      _assert("B6: Disabled user → USER_DISABLED", !disabledAuth.success && disabledAuth.code === ERROR_CODES.USER_DISABLED,
        "Expected USER_DISABLED, got " + disabledAuth.code);
    }
  }

  // B7: Invalid category for expense (SHOULD FAIL)
  var badCat = _parseResponse(ExpenseService.create(admin, {
    category: "Bitcoin Mining", description: "Bad", amount: 1000
  }));
  _assert("B7: Invalid category → INVALID_CATEGORY", !badCat.success && badCat.code === ERROR_CODES.INVALID_CATEGORY,
    "Expected INVALID_CATEGORY, got " + badCat.code);

  // B8: Missing required fields (SHOULD FAIL)
  var missingFields = _parseResponse(DonationService.create(admin, { donorName: "" }));
  _assert("B8: Missing fields → MISSING_FIELD", !missingFields.success,
    "Expected validation failure, got success");

  // B9: Negative amount (SHOULD FAIL)
  var negAmt = _parseResponse(DonationService.create(admin, {
    donorName: "Test", amount: -500, category: "General", paymentMode: "Cash"
  }));
  _assert("B9: Negative amount → INVALID_AMOUNT", !negAmt.success && negAmt.code === ERROR_CODES.INVALID_AMOUNT,
  ...
  _assert("B10: Dispatch without email → UNAUTHORIZED", !noEmail.success && noEmail.code === ERROR_CODES.UNAUTHORIZED,
...
function _runGroupC() {
  ...
  var don1 = _parseResponse(DonationService.create(admin, {
    donorName: "Finance Test A", amount: 500, category: "General", paymentMode: "Cash"
  }));
  _assert("C1: Create ₹500 Donation", don1.success, don1.message);
  var don1Id = don1.data.id;

  // C2: Create Donation ₹1000
  var don2 = _parseResponse(DonationService.create(admin, {
    donorName: "Finance Test B", amount: 1000, category: "General", paymentMode: "UPI"
  }));
  _assert("C2: Create ₹1000 Donation", don2.success, don2.message);
  var don2Id = don2.data.id;

  // C3: Create Expense ₹200
  var exp1 = _parseResponse(ExpenseService.create(admin, {
    category: "Decoration", description: "Flowers", amount: 200
  }));
  _assert("C3: Create ₹200 Expense", exp1.success, exp1.message);

  // C4: Create Expense ₹300
  var exp2 = _parseResponse(ExpenseService.create(admin, {
    category: "Transportation", description: "Taxi", amount: 300
  }));
  ...
  _assert("C5: Balance after 4 ops = +" + 1000,
  ...
  _parseResponse(ExpenseService.cancel(admin, { expenseId: exp2Id, reason: "Test cancel" }));
  var after2 = getBalance();
  _assert("C6: Cancel ₹300 expense → Balance +" + 1300,
  ...
  _assert("C9: Cash + UPI = Total Donations",
    after3.donations.cash + after3.donations.upi === after3.donations.total,
    "Cash " + after3.donations.cash + " + UPI " + after3.donations.upi + " != " + after3.donations.total);
}
...
function _runGroupD() {
  ...
  var donIds = [];
  for (var i = 0; i < 5; i++) {
    var res = _parseResponse(DonationService.create(admin, {
      donorName: "Integrity Test " + i, amount: 100 + i, category: "General", paymentMode: "Cash"
    }));
    if (res.success) donIds.push(res.data.id);
  }
  var uniqueDonIds = donIds.filter(function(v, i, a) { return a.indexOf(v) === i; });
  _assert("D1: 5 Donation IDs are unique", uniqueDonIds.length === 5,
    "Expected 5 unique, got " + uniqueDonIds.length);

  var expIds = [];
  for (var j = 0; j < 5; j++) {
    var expRes = _parseResponse(ExpenseService.create(admin, {
      category: "Decoration", description: "Integrity Exp " + j, amount: 50 + j
    }));
    if (expRes.success) expIds.push(expRes.data.id);
  }
  var uniqueExpIds = expIds.filter(function(v, i, a) { return a.indexOf(v) === i; });
  _assert("D2: 5 Expense IDs are unique", uniqueExpIds.length === 5,
    "Expected 5 unique, got " + uniqueExpIds.length);

  // D3: User IDs are unique
  var userIds = [];
  for (var k = 0; k < 3; k++) {
    var userRes = _parseResponse(UserService.createUser({
      fullName: "Integrity User " + k, email: "intuser" + k + "@gpms.org",
      phone: "800000000" + k, role: CONFIG.roles.volunteer, status: CONFIG.status.active
    }));
    if (userRes.success) userIds.push(userRes.data.id);
  }
  var uniqueUserIds = userIds.filter(function(v, i, a) { return a.indexOf(v) === i; });
  _assert("D3: User IDs are unique", uniqueUserIds.length === userIds.length,
  ...
  _assert("D4: Audit logs exist for donations", donationAudits > 0,
  ...
  _assert("D5: Audit logs exist for expenses", expenseAudits > 0,
  ...
  _assert("D6: No donation has empty ID", emptyDonIds === 0,
  ...
  _assert("D7: No expense has empty ID", emptyExpIds === 0,
...
function _runGroupE() {
  ...
  _assert("E1: Public dashboard accessible without auth", pubRes.success, pubRes.message);

  // E2: Public dashboard returns committee name
  _assert("E2: Public dashboard has committeeName",
    pubRes.success && pubRes.data.committeeName !== undefined,
    "Missing committeeName");

  // E3: Public dashboard returns year
  _assert("E3: Public dashboard has year",
    pubRes.success && pubRes.data.year !== undefined,
    "Missing year");

  // E4: Public dashboard returns balance
  _assert("E4: Public dashboard has balance",
    pubRes.success && pubRes.data.balance !== undefined,
    "Missing balance");

  // E5: Public dashboard does NOT expose donor names
  _assert("E5: No donor names in public dashboard",
    pubRes.success && pubRes.data.donors === undefined && pubRes.data.donorName === undefined,
    "PII leak: donor names exposed");

  // E6: Public dashboard does NOT expose phone numbers
  _assert("E6: No phone numbers in public dashboard",
    pubRes.success && pubRes.data.phone === undefined && pubRes.data.phones === undefined,
    "PII leak: phone numbers exposed");

  // E7: Public dashboard does NOT expose remarks
  _assert("E7: No remarks in public dashboard",
  ...
  var searchFail = _parseResponse(dispatch("searchDonations", { donorName: "test" }));
  _assert("E8: Unauthenticated searchDonations → UNAUTHORIZED",
  ...
  _assert("E9: Unauthenticated getAllUsers → UNAUTHORIZED",
  ...
  var createFail = _parseResponse(dispatch("createDonation", {
    donorName: "Hacker", amount: 999, category: "General", paymentMode: "Cash"
  }));
  _assert("E10: Unauthenticated createDonation → UNAUTHORIZED",
...
function _runRecoveryTest() {
  ...
  if (donRes.success) {
    var getRes = _parseResponse(DonationService.get(admin, { donationId: donRes.data.id }));
    _assert("R2: Donation retrievable after creation",
